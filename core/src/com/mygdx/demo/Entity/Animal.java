...
public class Animal extends Enemy {
    ...
    public Animal(GameScreen screen, float x, float y) {
        super(screen, x, y);
        frames = new Array<TextureRegion>();

        frames.add(new TextureRegion(screen.getAtlas().findRegion("newCharacters/bugDeadcopy"), 101, 1, 48, 48));
    ...
    public void update(float dt) {
        ...
        if (setToDestroy && !destroyed){
            ...
            destroyed = true;
           setRegion(new TextureRegion(screen.getAtlas().findRegion("newCharacters/bugDeadcopy"),101,1,0,0));
           stateTime=0;
        }
        else if (!destroyed) {
